...
class Eye(object):
    ...
    def detectPupil(self):
        nb_blocs_v = 14
        nb_blocs_h = 16
        bloc_h = self.h // nb_blocs_v
        bloc_w = self.w // nb_blocs_h
        reduced_h = bloc_h * nb_blocs_v
        reduced_w = bloc_w * nb_blocs_h

        bloc_means = np.mean(np.mean((self.gray[:reduced_h,:reduced_w]).reshape(nb_blocs_v, bloc_h, reduced_w), 1).reshape(nb_blocs_h*nb_blocs_v, bloc_w), 1)
        threshold = np.min(bloc_means)
        darker_bloc_pos = np.argmin(bloc_means)
        darker_bloc_x = (darker_bloc_pos % nb_blocs_h) * bloc_w
        darker_bloc_y = (darker_bloc_pos // nb_blocs_h) * bloc_h

        binary_frame = (np.where(self.gray <= threshold, 1, 0) * 200).astype('uint8')
